...
const go = async () => {
	...
	const startProcess = path => {
		...
		logger.info(`🥾  Starting node app 'gatekeeper', process id is #${process.pid}.`)
	...
	bundler.on('buildStart', () => { logger.info(`🐌  Started building 'gatekeeper' server bundle.`) })
	bundler.on('bundled', async () => {
		logger.info(`🎉  Finished building 'gatekeeper' server bundle.`)
		killProcesses()
		try {
			startProcess(Path.join(__dirname, './build/main.js'))
		} catch (error) {
			logger.error(error, 'the \'gatekeeper\' server bundle won\'t run 🤷‍♂️')
		}
	});
